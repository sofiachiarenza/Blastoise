...
class CalculatorFKEM(CalculatorBase):
    ...
    def setup(self):
        ...
        dpk = self.get_pk()
        a = 1./(1+dpk['z'][::-1])
        self.cosmo = ccl.CosmologyCalculator(
            ...
            A_s=par['A_s'],
            w0=par['w0'],
            pk_linear={'a': a, 'k': dpk['k'], 'delta_matter:delta_matter': dpk['pk_lin'][::-1][:]},
            pk_nonlin={'a': a, 'k': dpk['k'], 'delta_matter:delta_matter': dpk['pk_nl'][::-1][:]}
        )

        self.cosmo_nonlin_part = ccl.CosmologyCalculator(
            Omega_c=par['Omega_m']-par['Omega_b'],
            Omega_b=par['Omega_b'],
            h=par['h'],
            n_s=par['n_s'],
            ...
            w0=par['w0'],
            pk_linear={'a': a, 'k': dpk['k'], 'delta_matter:delta_matter': dpk['pk_lin'][::-1][:]},
            pk_nonlin={'a': a, 'k': dpk['k'], 'delta_matter:delta_matter': (dpk['pk_nl'][::-1][:]-dpk['pk_lin'][::-1][:])}
        )
        ...
        if self.config.get('tracers_from_kernels', False):
            tpar = self.get_tracer_parameters()  # Extract the bias array
            ...
            for k in ker['kernels_cmb']:
                t = ccl.Tracer()
                t.add_tracer(self.cosmo, kernel=(ker['chi_sh'], k), der_bessel=-1, der_angles=1)
                self.t_k.append(t)

                ker_at_chi = interp1d(np.log(ker['chi_sh']), k, bounds_error=False, fill_value=0.)
                ker_logspace = ker_at_chi(np.log(self.chi_logspace_arr)) * self.chi_logspace_arr * growfac_arr
                self.t_k_logspace.append(ker_logspace)
            ...
            z_s = nzs['z_sh']
            self.t_g = [
                ccl.NumberCountsTracer(self.cosmo, True, (z_g, nzs['dNdz_cl'][:, ni]), bias=(z_g, np.full(len(z_g), b)))
                for ni, b in zip(range(0, 10), tpar['b_g'])
            ]
            self.t_s = [
                ccl.WeakLensingTracer(self.cosmo, (z_s, nzs['dNdz_sh'][:, ni]), True)
                for ni in range(0, 5)
            ]

        self.fk_g = [None] * len(self.t_g)
        self.fk_s = [None] * len(self.t_s)
        self.fk_k = [None] * len(self.t_k)

    def _get_cl_limber(self, t1, t2, ls):
        return ccl.angular_cl(self.cosmo, t1, t2, ls, limber_integration_method='qag_quad')
    ...
    def _get_cls_nonlimber_lin(self, fchi1, fchi2, ls, cl_type, i1, i2):
        ...
        if cl_type == "gg":
            ...
            if self.fk_g[i1+i2] is None:
                myfftlog2 = fftlog(self.chi_logspace_arr, fchi2, nu=nu, N_extrap_low=0, N_extrap_high=0, c_window_width=0.25, N_pad=Npad2)
        ...
        if cl_type == "kg":
            ...
            if self.fk_k[i2] is None:
                myfftlog2 = fftlog(self.chi_logspace_arr, fchi2, nu=nu2, N_extrap_low=0, N_extrap_high=0, c_window_width=0.25, N_pad=Npad)
                self.k_logspace_arr, self.fk_k[i2] = myfftlog2.fftlog_modified_ells(ls)
            fk2 = self.fk_k[i2]
        ...
        if cl_type == "ks":
            if self.fk_s[i1] is None:
                myfftlog1 = fftlog(self.chi_logspace_arr, fchi1, nu=nu, N_extrap_low=0, N_extrap_high=0, c_window_width=0.25, N_pad=Npad)
                self.k_logspace_arr, self.fk_s[i1] = myfftlog1.fftlog_ells(ls)
            fk1 = self.fk_s[i1]
            ...
            prefac = (ls+1)*(ls)*np.sqrt((ls+2)*(ls+1)*(ls)*(ls-1))
            prefac[prefac < 0] = 0.
    ...
    def _get_cl_nonlimber_fang(self, t1, t2, ls, i1, i2, cl_type):
        ...
        if cl_type == 'kg':
            ls_nonlim = ls[ls < l_switch]
            cls_limber_nonlin_part = self._get_cl_limber_nonlin_part(t1, t2, ls_nonlim)
            cls_nonlimber_lin = self._get_cls_nonlimber_lin(self.t_k_logspace[i1], self.t_g_logspace[i1+i2], ls_nonlim, cl_type, i1, i2)
            cls_nonlimber = cls_limber_nonlin_part + cls_nonlimber_lin
            cls_limber = self._get_cl_limber(t1, t2, ls[ls >= l_switch])
            return np.hstack((cls_nonlimber, cls_limber))
        ...
        if cl_type == 'ks':
            ls_nonlim = ls[ls < l_switch]
            cls_limber_nonlin_part = self._get_cl_limber_nonlin_part(t1, t2, ls_nonlim)
            cls_nonlimber_lin = self._get_cls_nonlimber_lin(self.t_k_logspace[i1], self.t_s_logspace[i1+i2], ls_nonlim, cl_type, i1, i2)
            cls_nonlimber = cls_limber_nonlin_part + cls_nonlimber_lin
            cls_limber = self._get_cl_limber(t1, t2, ls[ls >= l_switch])
            return np.hstack((cls_nonlimber, cls_limber))
    ...
    def run(self):
        ...
        for i1, t1 in enumerate(self.t_k):
            for i2, t2 in enumerate(self.t_g):
                self.cls_kg.append(self._get_cl_nonlimber_fang(t1, t2, ls, i1, i2, 'kg'))
            for i2, t2 in enumerate(self.t_s):
                self.cls_ks.append(self._get_cl_nonlimber_fang(t1, t2, ls, i1, i2, 'ks'))

        self.cls_gg = np.array(self.cls_gg)
        self.cls_gs = np.array(self.cls_gs)
        self.cls_ss = np.array(self.cls_ss)
        self.cls_kg = np.array(self.cls_kg)
        self.cls_ks = np.array(self.cls_ks)

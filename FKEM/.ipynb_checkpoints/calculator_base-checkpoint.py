...
class CalculatorBase(object):
    ...
    def get_num_cls(self):
        ...
        nkg = self.nb_g * self.nb_k
        nks = self.nb_s * self.nb_k
        return ngg, ngs, nss, nkg, nks

    def write_output(self):
        ls = self.get_ells()
        nl = len(ls)
        ngg, ngs, nss, nkg, nks = self.get_num_cls()
        ...
        if self.cls_kg.shape != (nkg, nl):
            raise ValueError("Incorrect K-G spectra shape")
        if self.cls_ks.shape != (nks, nl):
            raise ValueError("Incorrect K-S spectra shape")
        ...
        np.savez(self.config['output_prefix'] + '_clkg.npz',
                 ls=ls, cls=self.cls_kg)
        np.savez(self.config['output_prefix'] + '_clks.npz',
                 ls=ls, cls=self.cls_ks)
    ...
    def run(self):
        ...
        self.cls_kg = np.zeros((nkg, nl))
        self.cls_ks = np.zeros((nks, nl))